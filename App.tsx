...
const App: React.FC = () => {
  ...
  return (
    <div className="min-h-screen font-sans overflow-x-hidden flex flex-col relative selection:bg-orange-500/30 selection:text-black dark:selection:text-white transition-colors duration-500">

      {/* Dark Mode Background */}
      <div className="fixed inset-0 z-[-1] pointer-events-none">
        <div className="absolute inset-0 bg-[#0A0A0F]">
          <div
            className="absolute inset-0 bg-cover bg-center bg-no-repeat bg-fixed opacity-60 mix-blend-overlay"
            style={{ backgroundImage: "url('https://images.unsplash.com/photo-1516035069371-29a1b244cc32?q=80&w=2564&auto=format&fit=crop')" }}
          />
          <div className="absolute inset-0 bg-gradient-to-b from-[#0A0A0F]/40 via-[#0A0A0F]/30 to-[#0f0f14]/50" />
        </div>
      </div>
      ...
      {comparisonList.length > 0 && (
        <div className="fixed bottom-24 left-1/2 -translate-x-1/2 z-40 bg-white dark:bg-[#15151A] rounded-full shadow-2xl border border-gray-200 dark:border-gray-700 p-2 pl-6 flex items-center gap-4 animate-fade-in-up">
          <span className="text-sm font-medium text-gray-900 dark:text-white">
            Đang chọn so sánh: <span className="font-bold text-red-600">{comparisonList.length}/3</span>
          </span>
          <button
