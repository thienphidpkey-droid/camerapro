    ...
    <section className="relative min-h-screen w-full overflow-hidden flex items-center justify-center bg-transparent">
      ...
      <div className="container relative z-20 px-6 md:px-12 mt-16 grid grid-cols-1 lg:grid-cols-12 gap-12 items-center h-full">
         ...
         <div className="lg:col-span-5 flex flex-col justify-center animate-fade-in-up delay-300">
            <div className="bg-white/60 dark:bg-black/40 backdrop-blur-xl border border-gray-200 dark:border-white/10 rounded-2xl p-8 shadow-2xl">
                ...
                <div className="grid grid-cols-2 gap-4">
                    {brands.map((brand) => (
                        <div 
                            key={brand}
                            className="group relative h-20 bg-gray-50 dark:bg-black/40 hover:bg-white dark:hover:bg-white/10 border border-gray-200 dark:border-white/5 hover:border-purple-500/50 rounded-xl flex items-center justify-center transition-all duration-300 cursor-pointer overflow-hidden shadow-sm dark:shadow-none"
                            onClick={() => onNavigate('catalogue')}
                        >
                            <span className="text-gray-600 dark:text-gray-300 font-medium group-hover:text-purple-600 dark:group-hover:text-white tracking-wide z-10 relative group-hover:scale-110 transition-transform">
                                {brand}
                            </span>
                            {/* Hover glow effect */}
                ...
                <div 
                    className="mt-6 flex items-center justify-between text-gray-500 dark:text-gray-400 text-sm hover:text-purple-600 dark:hover:text-white cursor-pointer transition-colors group"
                    onClick={() => onNavigate('catalogue')}
                >
